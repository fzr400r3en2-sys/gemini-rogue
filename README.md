...
## 安全設計
- **読み取り専用**: ファイルの削除、移動、名前の変更、内容の書き換えは一切行いません。通常時はファイル名やサイズ等のメタデータのみを使用し、ファイル本文の読み取りは行いません。
- **プライバシー配慮**: デフォルトではファイル名とメタデータのみを使用します。ハッシュ計算オプション（`--hash-duplicates`）を有効にした場合はファイル内容を読み取ってSHA-256を計算するためディスクI/Oが発生しますが、ファイル内容を保存したり外部に送信したりすることはありません。
...
- `--hash-duplicates`: ファイルサイズが同一の場合にファイル内容を読み取り、SHA-256ハッシュ値を計算して厳密な重複チェックを行います（ディスクI/Oが発生します。デフォルトはサイズと名前で判定し、ファイル本文は読み取りません）。
- `--top-n <N>`: 各ランキング（大容量ファイル等）の表示件数（デフォルト: 20）。
- `--max-depth <N>`: 探索するディレクトリの最大深さ（0は対象フォルダ直下のみ、1は1階層下のフォルダまで、未指定時は無制限）。
